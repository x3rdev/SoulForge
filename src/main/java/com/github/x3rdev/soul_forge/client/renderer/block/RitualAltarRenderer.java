...
public class RitualAltarRenderer extends GeoBlockRenderer<RitualAltarBlockEntity> {
    ...
    public static final int RITUAL_DURATION = RitualAltarBlockEntity.RITUAL_DURATION;
    ...
    private void renderRitual(PoseStack poseStack, RitualAltarBlockEntity animatable, BakedGeoModel model, MultiBufferSource bufferSource, @Nullable VertexConsumer buffer, float partialTick, int packedLight, int packedOverlay, int colour) {
        poseStack.pushPose();
    ...
    private static Vec3 getRitualTranslations(RitualAltarBlockEntity animatable, float partialTick) {
        if(animatable.getRitualTicks() < 40) {
            return Vec3.ZERO;
        } else {
            Vec3 finalTarget = new Vec3(0, 1.5, 0);
            float scale = Math.min(0,(animatable.getRitualTicks()+partialTick-200F)/160F);
            return finalTarget.add(finalTarget.scale(scale));
        }
    }

    @Override
    public AABB getRenderBoundingBox(RitualAltarBlockEntity blockEntity) {
        if(blockEntity.isRitualActive()) {
            BlockPos pos = blockEntity.getBlockPos();
            return new AABB(pos.getX() - 3.0, pos.getY(), pos.getZ() - 3.0, pos.getX() + 4.0,
                    pos.getY() + 1.0, pos.getZ() + 4.0);
        } else {
